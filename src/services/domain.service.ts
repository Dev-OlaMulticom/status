 ...
 */
export async function enrichDomain(domain: string, serverIp: string | null): Promise<DomainEnrichment> {
  const [dns, whois, ssl, cfZone] = await Promise.all([
    resolveDomain(domain).catch((e) => { logger.debug({ domain, error: e.message }, 'DNS enrich failed'); return null; }),
    getWhoisInfo(domain).catch((e) => { logger.debug({ domain, error: e.message }, 'WHOIS enrich failed'); return null; }),
    checkSsl(domain).catch((e) => { logger.debug({ domain, error: e.message }, 'SSL enrich failed'); return null; }),
    getZoneForDomain(domain).catch(() => null),
  ]);

  const ipMatchesServer =
    serverIp != null && dns?.ipv4 != null
      ? dns.ipv4 === serverIp
      : null;
...
export async function enrichDomains(
  ...
  return Promise.all(
    domains.map((d) => limit(() => enrichDomain(d, serverIp))),
  );
}
